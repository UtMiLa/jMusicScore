...
import { IScore, IGlobalContext } from './model/jm-model-interfaces';
                ...
                export class CanvasGraphicsEngine implements IGraphicsEngine {
                    ...
                    public createMusicObject(id: string, item: string, x: number, y: number, scale: number): any {
                        ...
                        this.context.scale(1 / scale, 1 / scale);
                            ...
                            var char: string = fontCodePoints[item.substr(2)];
                            if (char === undefined) {
                                char = 'u';
                            }
                    ...
                    private drawPath(path: string, fillStyle: string, strokeStyle: string): any {
                        function move(stack: number[], context: CanvasRenderingContext2D, current: Point, relative: boolean): boolean {
                        ...
                        function line(stack: number[], context: CanvasRenderingContext2D, current: Point, relative: boolean): boolean {
                            if (stack.length < 2) return false;
                        ...
                        function horiz(stack: number[], context: CanvasRenderingContext2D, current: Point, relative: boolean): boolean {
                        ...
                        function vert(stack: number[], context: CanvasRenderingContext2D, current: Point, relative: boolean): boolean {
                            if (stack.length < 1) return false;
                        ...
                        function curve(stack: number[], context: CanvasRenderingContext2D, current: Point, relative: boolean): boolean {
                        ...
                        function close(stack: number[], context: CanvasRenderingContext2D, current: Point, relative: boolean): boolean {
                            context.closePath();
                            return true;
                        }
        
                        var operations: { [index: string]: (stack: number[], context: CanvasRenderingContext2D, current: Point, relative: boolean) => boolean } = {
                            'm': move,
                            'M': move,
                            'c': curve,
                            'C': curve,
                        ...
                        var operation: (stack: number[], context: CanvasRenderingContext2D, current: Point, relative: boolean) => boolean = move;
                        var stack: number[] = [];
                        var current = { x: 0, y: 0 };
                        var relative = false;
                ...
                class HtmlGraphicsEngine implements IGraphicsEngine, ISensorGraphicsEngine {
                    ...
                    public calcCoordinates(event: MouseEvent): Point {
                        ...
                        if (event.offsetY === undefined) {
                            ...
                            var offset = DomHelper.getOffset(elm);
                            offsetY = (event.clientY - offset.top) * elm.offsetHeight / rect.height;
                            offsetX = (event.clientX - offset.left) * elm.offsetHeight / rect.height;
                        }
        
                        var p = new Point(offsetX, offsetY + parseInt(DomHelper.getCss(<HTMLElement>event.target,'top')));
                        return p;
                    }
...
*/
                ...
                /*
                export class CanvasViewer implements IScorePlugin {
